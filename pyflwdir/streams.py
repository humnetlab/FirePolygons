...
# general methods
@njit
def accuflux(idxs_ds, seq, data, nodata, count=None):
    ...
    accu = data.copy().astype(np.int64)

    if count==1:
        print("Accumulation by count")
        accu_thresh = accu > 0

        accu = (accu_thresh * 1)
        accu[accu <= 0] = 0  # Set nodata cells to 0
        accu = accu.ravel()

        for idx0 in seq[::-1]:  # From upstream to downstream
            idx_ds = idxs_ds[idx0]
            if idx0 != idx_ds and accu[idx_ds] != nodata and accu[idx0] != nodata:
                accu[idx_ds] += 1

    elif count==2:
        print("Accumulation by count with no repeat")
        accu_thresh = accu > 0
        accu = (accu_thresh * 1)
        accu[accu <= 0] = 0  # Set nodata cells to 0
        accu = accu.ravel()

        visited = np.zeros(accu.size, dtype=np.bool_)  # Track visited cells with correct dtype

        for idx0 in seq[::-1]:  # From upstream to downstream
            idx_ds = idxs_ds[idx0]
            if idx0 != idx_ds and accu[idx_ds] != nodata and accu[idx0] != nodata:
                if not visited[idx_ds]:  # Check if downstream cell has been visited
                    accu[idx_ds] += accu[idx0]
                    visited[idx_ds] = True  # Mark downstream cell as visited
