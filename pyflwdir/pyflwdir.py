...
def from_dem(
    ...
    fdir: {'d8', 'dinf'}
        D8 and D-infinity flow direction processing
    ...
    if fdir == "d8":
        ...
        dinf = dem.fill_depressions_dinf(
            data, nodata=nodata, max_depth=max_depth, outlets=outlets
        )
        return dinf
...
class FlwdirRaster(Flwdir):
    ...
    def subbasins_pfafstetter(self, depth=1, uparea=None, upa_min=0.0, n_largest_basins=4):
        ...
        subbas, idxs_out = basins.subbasins_pfafstetter(
            ...
            depth=depth,
            mv=self._mv,
            n_largest_basins=n_largest_basins
        )
        return subbas.reshape(self.shape), idxs_out
    ...
    def subbasins_pfafstetter_with_roads(self, 
                                         depth=1, 
                                         uparea=None, 
                                         upa_min=0.0, 
                                         n_largest_basins=4,
                                         roads_gdf=None,
                                         road_suitability_attr=None,
                                         road_threshold=None):
        """Returns the pfafstetter subbasins.
        ...
        if upa_min is not None:
            mask = uparea >= upa_min
        subbas, idxs_out = basins.subbasins_pfafstetter_with_roads(
            idxs_pit=self.idxs_pit,
            idxs_ds=self.idxs_ds,
            idxs_us_main=self.idxs_us_main,
            seq=self.idxs_seq,
            ...
            n_largest_basins=n_largest_basins,
            roads_gdf=roads_gdf, 
            road_suitability_attr=road_suitability_attr, 
