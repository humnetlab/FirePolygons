...
def subbasins_pfafstetter(
    idxs_pit, idxs_ds, seq, idxs_us_main, uparea, mask=None, depth=1, mv=_mv, n_largest_basins=4
):
    ...
    while len(labs) > 0:
        ...
        idxs_trib0 = idxs0s[:n_largest_basins]
...
@njit
def subbasins_pfafstetter_with_roads(
    idxs_pit, idxs_ds, seq, idxs_us_main, uparea, roads_gdf, road_suitability_attr, road_threshold,
    mask=None, depth=1, mv=_mv, n_largest_basins=4
):
    strord = streams.stream_order(idxs_ds, seq, idxs_us_main, mask=mask, mv=mv)
    strord = np.where(strord <= depth + 1, strord, 0).astype(strord.dtype)
    idxs_trib = _tributaries(idxs_ds, seq, strord)
    
    # Initialize Pfafstetter branches with zero
    pfaf_branch = np.zeros(idxs_ds.size, np.int32)
    idxs = []
    labs = [(int(0), int(0)) for _ in range(0)]  # set dtypes
    
    # Iterate through road segments to enforce usage based on suitability score
    road_segments_used = np.zeros(idxs_ds.size, dtype=np.int32)  # mark used road segments
    for road_idx, road_segment in roads_gdf.iterrows():
        suitability = road_segment[road_suitability_attr]
        if suitability >= road_threshold:
            road_coords = np.array(list(road_segment.geometry.coords))
            for coord in road_coords:
                road_cell_idx = get_raster_cell_index_from_coord(coord, idxs_ds.shape)
                if road_cell_idx != -1:
                    road_segments_used[road_cell_idx] = 1
                    pfaf_branch[road_cell_idx] = road_idx  # Mark this road cell as a boundary
    
    pfaf0 = 1
    for d0 in range(1, depth):
        pfaf0 += 10**d0
    
    # Delineate basins using the stream order
    for i, idx in enumerate(idxs_pit):
        idxs.append(idx)
        pfaf1 = pfaf0 + (i + 1) * 10**depth
        labs.append((pfaf1, 1))
        ...
        while True:
            idx = idxs_us_main[idx]
            if idx == mv or strord[idx] == 0 or road_segments_used[idx]:  # Stop at road segments
                break
            pfaf_branch[idx] = pfaf1
    ...
    while len(labs) > 0:
        ...
        for i, idx in enumerate(idxs_trib0s):
            idxs.append(idx)
            idx1 = idxs_us_main[idxs_ds[idx]]
            pfaf_sub = pfaf0 + (i * 2 + 1) * 10 ** (depth - d0)
            pfaf_branch[idx] = pfaf_sub
            
            while True:
                idx = idxs_us_main[idx]
                if idx == mv or strord[idx] == 0 or road_segments_used[idx]:
                    break
                pfaf_branch[idx] = pfaf_sub
            ...
            if idx1 not in idxs:
                idxs.append(idx1)
                pfaf_int = pfaf0 + (i + 1) * 2 * 10 ** (depth - d0)
                pfaf_branch[idx1] = pfaf_int
                while True:
                    idx1 = idxs_us_main[idx1]
                    if idx1 == mv or pfaf_branch[idx1] != pfaf_int_ds or road_segments_used[idx1]:
                        break
                    pfaf_branch[idx1] = pfaf_int
                pfaf_int_ds = pfaf_int
                if d0 < depth:
...
def get_raster_cell_index_from_coord(coord, raster_shape):
    # Convert coordinate to raster cell index
    # Implement logic to map coord to raster cell index
    pass
