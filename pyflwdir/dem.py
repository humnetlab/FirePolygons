...
def fill_depressions_dinf(
    ...
    """Fill local depressions in elevation data for D-infinity flow direction processing."""

    nrow, ncol = elevtn.shape
    delv = np.zeros_like(elevtn, dtype=np.float32)
    done = np.isnan(elevtn) if np.isnan(nodata) else elevtn == nodata
    
    if connectivity not in [4, 8]:
        raise ValueError(f'"connectivity" should either be 4 or 8')

    # Create boolean array for neighbor structure
    ...
    # initiate queue
    if idxs_pit is None:  # with edge cells
        queued = get_edge(~done, structure=struct)
        if elv_max is not None:
            queued = np.logical_and(queued, elevtn <= elv_max)
            if not np.any(queued):
                raise ValueError("No initial outlet cells found.")
    else:  # with user defined outlet cells
        queued = np.zeros((nrow, ncol), dtype=np.bool_)
        for idx in idxs_pit:
            queued.flat[idx] = True
    ...
    while len(q) > 0:
        z0, _, r0, c0 = heapq.heappop(q)
        for dr, dc in zip(drs, dcs):
            r = r0 + dr  # Ensure r0 and dr are both integers
            c = c0 + dc  # Ensure c0 and dc are both integers
            ...
            if not queued[r, c]:  # add to queue
                heapq.heappush(q, (z1, np.uint8(0), np.uint32(r), np.uint32(c)))
                queued[r, c] = True
...
@njit
def get_edge(mask, structure):
    """
    Identify edge pixels in a binary mask.
    """
    nrow, ncol = mask.shape
    edge = np.zeros_like(mask, dtype=np.bool_)
    ...
    for r in range(1, nrow-1):
        for c in range(1, ncol-1):
            if mask[r, c]:
                for dr in range(-1, 2):
                    for dc in range(-1, 2):
                        if structure[dr+1, dc+1] and not mask[r + dr, c + dc]:
                            edge[r, c] = True
                            break
                    if edge[r, c]:
                        break
